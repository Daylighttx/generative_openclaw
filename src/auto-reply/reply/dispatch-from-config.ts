...
const memoryLog = createSubsystemLogger("dispatch").child("memory");
...
function maybeRecordMemory(
  ...
  // Fire-and-forget: don't block reply delivery. Errors are logged inside bridge.
  mindOnInboundMessage(agentId, body, sender ?? undefined).catch((err) => {
    memoryLog.warn("memory recording failed", { agentId, error: String(err) });
  });
}
...
export async function dispatchReplyFromConfig(
  ...
  try {
    ...
    void maybeRecordMemory(ctx, sessionAgentId);
    markIdle("message_completed");
    return attachSourceReplyDeliveryMode({
      queuedFinal,
      counts,
