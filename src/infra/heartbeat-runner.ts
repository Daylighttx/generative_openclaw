...
export async function runHeartbeatOnce(opts: {
  ...
  const _heartbeatRunResult = resolveHeartbeatRunPrompt({
    cfg,
    heartbeat,
    preflight,
    canRelayToUser,
  ...
  let prompt: string | null = _heartbeatRunResult.prompt;
  ...
  if (
    ...
    !hasCronEvents &&
    !hasDueCommitments &&
    (!preflight.tasks || preflight.tasks.length === 0) &&
    !preflight.isWakePayload &&
    !preflight.isExecEventWake &&
    !preflight.isCronWake &&
    isHeartbeatContentEffectivelyEmpty(preflight.heartbeatFileContent)
  ) {
    prompt = null;
  }
  ...
  // Always advance AgentMind state on every heartbeat (mood decay, thinking counter, etc.)
  const mindTickResult = await mindOnHeartbeat(agentId);

  // If heartbeat has other tasks (prompt !== null) but agent wants to send a proactive message,
  // buffer it for the next idle heartbeat instead of discarding.
  // IMPORTANT: Check hasPendingMessage first — if the bridge already internally re-buffered a
  // newer action (because it consumed an old pending and produced a fresh one), we must NOT
  // overwrite it with the stale effectiveAction returned here.
  if (prompt !== null && mindTickResult && mindTickResult.shouldMessage && mindTickResult.thoughtAction) {
    if (!hasPendingMessage(agentId)) {
      bufferProactiveMessage(agentId, mindTickResult.thoughtAction);
      log.info("heartbeat: proactive message buffered (heartbeat busy): " + JSON.stringify({
        agentId,
        thoughtType: mindTickResult.thoughtAction.type,
        reason: mindTickResult.thoughtAction.reason ?? "(none)",
      }));
    } else {
      log.info("heartbeat: proactive message skipped buffering (newer action already pending): " + JSON.stringify({
        agentId,
        thoughtType: mindTickResult.thoughtAction.type,
      }));
    }
  ...
  const isProactiveMessage =
    prompt === null &&
    mindTickResult &&
    mindTickResult.shouldMessage &&
    mindTickResult.thoughtAction !== null;

  // If no tasks are due, check if AgentMind wants to send a proactive message
  if (prompt === null) {
    if (mindTickResult && mindTickResult.shouldMessage && mindTickResult.thoughtAction) {
      // AgentMind overrides: send a proactive message even though no HEARTBEAT.md tasks are due
      prompt = mindTickResult.thoughtAction.prompt;
      // For proactive messages, find delivery target by scanning sessions.json
      // for the most recent entry with a lastChannel (skipping heartbeat entries).
      const sessionInfo = resolveHeartbeatSession(cfg, agentId, heartbeat, undefined);
      const sessionStore = sessionInfo.store;
      let bestEntry: SessionEntry | undefined;
      let bestUpdated = 0;
      for (const [key, entry] of Object.entries(sessionStore)) {
        if (!entry || key.includes(":heartbeat") || !entry.lastChannel || !entry.lastTo) continue;
        if ((entry.updatedAt ?? 0) > bestUpdated) {
          bestUpdated = entry.updatedAt ?? 0;
          bestEntry = entry;
        }
      }
      if (bestEntry && bestEntry.lastChannel && bestEntry.lastTo) {
        delivery = {
          channel: bestEntry.lastChannel,
          to: bestEntry.lastTo,
          accountId: bestEntry.lastAccountId,
          threadId: bestEntry.lastThreadId,
          reason: "proactive-scan",
        };
      }
      log.info("heartbeat: proactive override complete: " + JSON.stringify({
        agentId,
        mood: mindTickResult.thoughtAction.moodDescription,
        thoughtType: mindTickResult.thoughtAction.type,
        channel: delivery.channel,
        to: delivery.to ?? "(none)",
        promptLen: prompt.length,
        memoryCount: mindTickResult.state.memoryCount,
        proactiveUrgency: Number(mindTickResult.state.proactiveUrgency.toFixed(3)),
        shouldMessage: mindTickResult.state.shouldMessage,
      }));
    } else {
  ...
  if (!visibility.showAlerts && !visibility.showOk && !visibility.useIndicator) {
    log.warn("heartbeat: all visibility disabled, skipping send", {
      agentId,
      channel: delivery.channel !== "none" ? delivery.channel : "(none)",
    });
    emitHeartbeatEvent({
      status: "skipped",
  ...
  try {
    ...
    const replyOpts = {
      ...
      ...(isProactiveMessage ? { disableTools: true, skillFilter: [] } : {}),
    ...
    if (isProactiveMessage && mindTickResult?.thoughtAction?.prompt) {
      const proactiveText = mindTickResult.thoughtAction.prompt.trim();
      log.info("heartbeat: proactive AI reply (direct): " + JSON.stringify({
        agentId,
        replyLen: proactiveText.length,
        replyPreview: proactiveText.slice(0, 200),
      }));
      if (proactiveText && delivery.channel !== "none" && delivery.to) {
        log.info("heartbeat: proactive delivery: " + JSON.stringify({
          agentId,
          hasText: true,
          textLen: proactiveText.length,
          textPreview: proactiveText.slice(0, 200),
          hasMedia: false,
          channel: delivery.channel,
          to: delivery.to,
        }));
        const send = await sendDurableMessageBatch({
          ...
          session: outboundSession,
          threadId: delivery.threadId,
          payloads: [{ text: proactiveText }],
          deps: opts.deps,
        });
        if (send.status === "failed" || send.status === "partial_failed") {
          throw send.error;
        }
        log.info("heartbeat: proactive message sent: " + JSON.stringify({
          agentId,
          channel: delivery.channel,
          to: delivery.to,
          textLen: proactiveText.length,
        ...
        log.info("heartbeat: proactive AI reply was empty or no delivery target, skipping", { agentId });
      }
    ...
    if (prompt && prompt.includes("Proactive Message Task")) {
      const replyText = typeof replyResult === "object" && replyResult && "text" in replyResult
        ? String((replyResult as { text: unknown }).text) : "";
      const strippedText = replyText.replace(/\n/g, "\\n");
      log.info("heartbeat: proactive AI reply: " + JSON.stringify({
        agentId,
        replyLen: replyText.length,
        replyPreview: strippedText.substring(0, 200),
        isArray: Array.isArray(replyResult),
        willBeFiltered: !strippedText.trim(),
      }));
    }
    ...
    if (isProactiveMessage) {
      const proactiveText = typeof replyPayload?.text === "string" ? replyPayload.text.trim() : "";
      const proactiveHasMedia = replyPayload
        ? resolveSendableOutboundReplyParts(replyPayload).hasMedia
        : false;
      log.info("heartbeat: proactive delivery: " + JSON.stringify({
        agentId,
        hasText: proactiveText.length > 0,
        textLen: proactiveText.length,
        textPreview: proactiveText.slice(0, 200),
        hasMedia: proactiveHasMedia,
        channel: delivery.channel,
        to: delivery.to ?? "(none)",
      }));
      if (proactiveText || proactiveHasMedia) {
        if (delivery.channel !== "none" && delivery.to) {
          const proactiveMediaUrls = replyPayload
          ...
          const send = await sendDurableMessageBatch({
            ...
            session: outboundSession,
            threadId: delivery.threadId,
            payloads: [{ text: proactiveText, mediaUrls: proactiveMediaUrls }],
            deps: opts.deps,
          });
          if (send.status === "failed" || send.status === "partial_failed") {
            throw send.error;
          ...
          emitHeartbeatEvent({
            status: "sent",
            to: delivery.to,
            preview: proactiveText.slice(0, 200),
            durationMs: Date.now() - startedAt,
            hasMedia: proactiveHasMedia,
            channel: delivery.channel,
            accountId: delivery.accountId,
          ...
          log.warn("heartbeat: proactive message has no delivery target", {
            agentId,
            channel: delivery.channel,
            to: delivery.to ?? "(none)",
          });
        ...
        log.info("heartbeat: proactive AI reply was empty, skipping delivery", { agentId });
      }
      await updateTaskTimestamps();
      consumeInspectedSystemEvents();
      return { status: "ran", durationMs: Date.now() - startedAt };
    ...
    if (delivery.channel === "none" || !delivery.to) {
      log.warn("heartbeat: no delivery target for proactive message", {
        agentId,
        channel: delivery.channel,
        to: delivery.to ?? "(none)",
